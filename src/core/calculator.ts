...
export interface CriminalResult {
  ...
  /** Custody: bail by the most serious crime, or null when there is none. */
...
/**
 * Combines criminal charges by the server's rules: the strictest punishment absorbs the rest,
 * an allowed article is added once on top, stages reduce each charge, the total is capped,
 * and the result says how many stars to set, what bail applies and which rules were used.
 */
export function calculateCriminal(charges: ChargeItem[], wanted: Mode, rules: CalculatorRules): CriminalResult {
  ...
  const stackArticle = rules.stackOnce?.article;
  ...
  if (strictest && absorbed.length) {
    explanation.push(`${labelOf(strictest.item)} поглощает ${absorbed.map((r) => labelOf(r.item)).join(', ')} — ${rules.absorption.basis}`);
  }
  if (stacked && rules.stackOnce) {
    explanation.push(`${labelOf(stacked.item)} добавлена к самому строгому один раз — ${rules.stackOnce.basis}; её можно и поглотить — ${rules.absorption.basis}`);
  }
  ...
  if (mode === 'custody' && strictest) {
    term = (strictest.term ?? 0) + (stacked?.term ?? 0);
    ...
    fineLimit = { min: strictest.fine.min, max: strictest.fine.max + (stacked?.fine?.max ?? 0) };
  }

  const stars = Math.min(rules.stars.max, Math.floor(term / rules.stars.monthsPerStar));
  ...
  return {
    ...
    capped,
    stars,
    starsMonths: stars * rules.stars.monthsPerStar,
    fineLimit,
    bail,
    explanation,
    warnings,
