...
- **Ищет как человек думает**: по номеру (`65`, `коап 8.6`, `ук 104 ч 2`), по словам и их формам, по жаргону (`ствол` → огнестрельное оружие) и с опечатками. Наказание, звёзды и подследственность видны прямо в результатах, а наказание по правилам проекта и уставам — красным.
- **Три сервера**: Тверской (59 документов), Арбатский (54) и Кутузовский (51). Конституция, УК, КоАП, ПДД, процессуальный и трудовой кодексы, ФКЗ и ФЗ, уставы и положения организаций, все общие правила проекта. Документы вашей организации — первыми.
...
- **Закреплённые карточки** — статьи и итог калькулятора остаются на экране поверх игры, а клики проходят сквозь них. Карточек может быть сколько угодно: каждая перетаскивается за свою шапку и растягивается за уголок, а если перетащить одну на другую — они соединяются в блок с той стороны, куда бросили: снизу, сверху или в ряд. Блок можно листать по одной карточке или свернуть в компактный вид, а набор закреплённых — сохранить и вернуть одной кнопкой. Закреплённые статьи возвращаются после перезапуска.
...
- **Законы обновляются сами**: правки с форума приходят с GitHub без новой версии программы, а после обновления программы видно, что нового во всех пропущенных версиях.
...
5. В настройках игры включите **«Оконный без рамки»** («Графика» → «Тип экрана») — в полноэкранном режиме Windows не даёт показывать окна поверх игры — и выключите **«Отключение звука при потере фокуса»** («Аудио»), иначе игра глушит звук, пока открыт хелпер.

Нужна Windows 10 или 11. Иконка программы живёт в трее: там же «Показать / скрыть» и «Выход».

## Обновления

Программа сама проверяет обновления при запуске и раз в несколько часов. Когда выйдет новая версия, в правом верхнем углу экрана на несколько секунд появится уведомление (его видно и поверх игры), а под заголовком оверлея — предложение обновиться: «Обновить» скачает и поставит её, программа перезапустится сама; «Позже» отложит до следующей версии. Проверить вручную — в настройках, кнопка «Проверить обновления».
...
- `src-tauri` — нативная часть: горячая клавиша, трей, окно закреплённых карточек, обновления
